...
- `HybridFlagProvider` registers its remote poll listener and remote/file change listeners only after both sub-providers have completed `init()`. The defensive `expectingSelfWrite.set(false)` reset inside `init()` was removed; under the new ordering the synchronous first poll inside `remote.init()` cannot reach `onPollComplete`, so the workaround is no longer needed.
