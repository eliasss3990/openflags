...
- `HybridFlagProvider` now writes snapshots on a dedicated `Executor` instead of the poller thread (ADR-3). The default executor is a single-threaded daemon named `openflags-snapshot-writer`; a custom one can be supplied via `HybridFlagProviderBuilder.snapshotExecutor(Executor)` (the caller owns its lifecycle in that case). Bursty polls coalesce into a single write of the latest snapshot via `AtomicReference.getAndSet(...)`; intermediate snapshots are intentionally dropped. `shutdown()` drains any pending snapshot synchronously, then shuts down the default executor with a 2-second bounded `awaitTermination`. See `docs/threading.md` for the full thread/callback contract.
