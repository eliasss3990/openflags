 ...
 * <p>
 * Contains the resolved value, the reason why that value was chosen, the flag
 * key,
 * and — when a {@code MultiVariantRule} matched — the variant label and the
 * matched
 ...
 * {@link io.github.eliasss3990.openflags.core.OpenFlagsClient#getStringResult(String, String, EvaluationContext)
 * getStringResult} or
 * {@link io.github.eliasss3990.openflags.core.OpenFlagsClient#getObjectResult(String, java.util.Map, EvaluationContext)
 ...
 * @param value         the resolved value, or the caller's default. May be
 *                      {@code null}
 *                      only when the caller's default is {@code null} and the
 *                      flag could
 ...
 * @param variant       the variant label when {@code reason == VARIANT},
 *                      otherwise {@code null}.
 *                      For string-typed flags this is the variant's raw string
 *                      value
 *                      (e.g. {@code "control"}, {@code "treatment-a"}); for
 *                      boolean/number
 *                      flags it is the stringified primitive; for object flags
 *                      it is {@code null}.
 * @param matchedRuleId the {@code name} of the rule that produced this result
 *                      when a rule
 *                      matched (reasons {@code TARGETING_MATCH}, {@code SPLIT},
 *                      {@code VARIANT}),
 *                      otherwise {@code null}.
...
public record EvaluationResult<T>(
    ...
    /**
     * Compact constructor enforcing the non-null invariants on
     * {@code reason} and {@code flagKey}.
     */
     ...
     * Use this instead of the canonical 5-arg constructor for error and
     * short-circuit
     * paths ({@code FLAG_NOT_FOUND}, {@code FLAG_DISABLED}, {@code PROVIDER_ERROR},
     * etc.).
     * Both {@code variant} and {@code matchedRuleId} will be {@code null} in the
     * returned result.
     *
