...
/**
 * Thrown when the type requested by the caller does not match the flag's actual
 * type.
 */
public class TypeMismatchException extends OpenFlagsException {

    /** The key of the flag involved (may be null when not applicable). */
    private final String flagKey;
    /** The type the caller requested. */
    private final FlagType expectedType;
    /** The type the flag actually has (may be null when unrecognized). */
    private final FlagType actualType;

    /**
     * Creates a type mismatch exception.
     *
     * @param flagKey      the key of the flag (may be null when used in value
     *                     construction)
     * @param expectedType the type the caller requested
     * @param actualType   the type the flag actually has (may be null for
     *                     unrecognized values)
     */
